 ...
 */
export async function saveModelConfig(id: string, config: ModelConfig): Promise<void> {
  try {
 ...
 * Decrypts API key if present
 */
export async function loadActiveConfig(): Promise<{ id: string; config: ModelConfig } | null> {
  try {
    const stored = loadAllConfigs();
...
export function loadAllConfigs(): StoredConfig {
  try {
    ...
    if (parsed.version !== STORAGE_VERSION) {
      console.warn(`Storage version mismatch: ${parsed.version} vs ${STORAGE_VERSION}. Call initializeEncryptedStorage() to migrate.`);
...
async function migrateStorage(old: StoredConfig): Promise<StoredConfig> {
  console.log(`Migrating storage from version ${old.version} to ${STORAGE_VERSION}`);
...
export async function initializeEncryptedStorage(): Promise<void> {
  if (!isSecureStorageAvailable()) {
    console.warn('Encrypted storage not available - Web Crypto API not supported');
    return;
  }

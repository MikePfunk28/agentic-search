  ...
  private createConfigFromEnv(
    ...
    try {
      ...
      const config: ModelConfig = {
        ...
        temperature: Number(process.env[`${envPrefix}_TEMPERATURE`] || defaults.temperature),
        maxTokens: Number(process.env[`${envPrefix}_MAX_TOKENS`] || defaults.maxTokens),
        timeout: Number(process.env[`${envPrefix}_TIMEOUT`] || 60000),
        enableStreaming: process.env[`${envPrefix}_ENABLE_STREAMING`] === "true",
      };

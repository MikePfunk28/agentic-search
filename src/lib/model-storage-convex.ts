...
function toConvexModelProvider(
	...
	switch (provider) {
		...
		case ModelProvider.AZURE_OPENAI:
			return provider;
		default:
			// Convex storage schema currently only supports a narrower provider set.
			// Treat unsupported OpenAI-compatible providers as OpenAI for persistence.
			return "openai";
	}
}
	...
	return async (configName: string, config: ModelConfig) => {
		try {
			...
			const configId = await createConfig({
				configName,
				provider: toConvexModelProvider(config.provider),
...
export function useActiveModelConfig() {
	...
	const apiKey = useQuery(
		api.secureApiKeys.getApiKey,
		activeConfig ? { configId: activeConfig._id } : "skip",
	);

...
export class QuerySegmenter {
	...
	constructor(
		...
		// SegmentationConfig merged for future use
	}
	...
	async segment(query: string): Promise<SegmentationResult> {
		...
		const estimatedTokens = segments.reduce(
		...
		console.log(
			`[Segmenter] Created ${segments.length} segments (${estimatedTokens} tokens estimated)`,
		);
	...
	private async analyzeQuery(query: string): Promise<QueryAnalysis> {
		...
		const hasComparison = /\b(compare|versus|vs|difference|better)\b/.test(
			lower,
		);
	...
	private async createSegments(
		...
		if (analysis.complexity === "complex") {
			console.log(
				`[Segmenter] Detected complex query - creating multi-part segmentation`,
			);
			...
			segments.push({
				...
				dependencies: [
					"intent",
					...segments.filter((s) => s.type === "entity").map((s) => s.id),
				],
				searchStrategy: "comparative",
				estimatedComplexity: "small",
				recommendedModel: this.suggestModel("small"),
		...
		segments.push({
			...
			recommendedModel: this.suggestModel(
				analysis.complexity as SegmentComplexity,
			),
			estimatedTokens: 400,
		});
	...
	private buildExecutionGraph(segments: QuerySegment[]): ExecutionGraph {
		...
		console.log(
			`[Segmenter] Execution graph: ${graph.totalStages} stages, ${graph.parallel.length} parallel groups`,
		);
	 ...
	 */
	private suggestModel(_complexity: SegmentComplexity): string {
	...
	private assessComplexity(
		...
		if (hasComparison || hasMultipleEntities || wordCount > 15)
			return "complex";
		return "moderate";
	}
	...
	private countEntities(query: string): number {
		...
		const potentialEntities = query.match(
			/[A-Z][a-z]+|\b(?:react|vue|node|python|javascript)\b/gi,
		);
	...
	private extractSimpleEntities(query: string): string[] {
		...
		const techTerms = query.match(
			/\b(?:React|Vue|Angular|Node|Python|JavaScript|TypeScript|Java|Go|Rust)\b/gi,
		);
	...
	private splitSequence(query: string): string[] {
		...
		const parts = query
			.split(/\b(?:then|after|next|following)\b/i)
			.map((s) => s.trim());
	...
	private estimateTime(
		...
		for (const stage of [...graph.parallel, ...graph.sequential]) {
			...
			const maxTime = Math.max(
				...stageSegments.map((s) => s.estimatedTokens / 50),
			); // ~50 tokens/sec
			totalTime += maxTime;
		}

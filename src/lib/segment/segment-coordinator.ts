...
export class SegmentCoordinator {
	...
	async execute(
		...
		console.log(
			`[Coordinator] Starting execution of ${segmentation.segments.length} segments`,
		);
		...
		try {
			...
			const allStages = [
				...segmentation.executionGraph.parallel.map((ids, i) => ({
					ids,
					parallel: true,
					stage: i,
				})),
				...segmentation.executionGraph.sequential.map((ids, i) => ({
					ids,
					parallel: false,
					stage: i,
				})),
			...
			const totalTokens = Object.values(state.segments).reduce(
				(sum, ctx) => sum + ctx.tokensUsed,
				0,
			);

			console.log(
				`[Coordinator] Execution complete: ${totalTime}ms, ${totalTokens} tokens`,
			);
			console.log(
				`[Coordinator] Success: ${state.completedSegments.size}/${segmentation.segments.length}`,
			);
	...
	private async executeParallel(
		...
		console.log(
			`[Coordinator] Executing ${segmentIds.length} segments in parallel`,
		);

		const segments = allSegments.filter((s) => segmentIds.includes(s.id));
		const promises = segments.map((segment) =>
			this.executeSegment(segment, state),
		);
	...
	private async executeSequential(
		...
		console.log(
			`[Coordinator] Executing ${segmentIds.length} segments sequentially`,
		);
	...
	private async executeSegment(
		...
		console.log(
			`[Coordinator] Executing segment ${segment.id}: "${segment.text}"`,
		);
		console.log(
			`[Coordinator] Context from ${Object.keys(relevantContext).length} dependencies`,
		);
	...
	private handleSuccess(
		...
		if (result.newSegments && result.newSegments.length > 0) {
			this.logAction(state, segment.id, "spawned_child", {
				count: result.newSegments.length,
			});
			console.log(
				`[Coordinator] Segment ${segment.id} spawned ${result.newSegments.length} new segments`,
			);
		}

		// Check if we need to escalate to a more powerful model
		if (result.shouldEscalate) {
			this.logAction(state, segment.id, "escalated", {
	...
	private synthesizeResults(state: CoordinationState): SearchResult[] {
		...
		for (const result of allResults) {
			if (
				...
				(result.addScore || 0) > (uniqueResults.get(result.url)?.addScore || 0)
			) {
	...
	private generateResponse(state: CoordinationState): string {
		...
		const totalSegments =
			Object.keys(state.segments).length + state.failedSegments.size;
	...
	): CoordinatedSearchResult["quality"] {
		const completedRatio =
			state.completedSegments.size /
			(state.completedSegments.size + state.failedSegments.size);
		const avgConfidence =
			Object.values(state.segments).reduce(
				(sum, ctx) => sum + ctx.findings.confidence,
				0,
	...
	private buildBreakdown(
		state: CoordinationState,
	): CoordinatedSearchResult["segmentBreakdown"] {
		...
		for (const [segmentId, _error] of state.failedSegments.entries()) {
			const execResult = state.executionResults?.get(segmentId);
			breakdown.push({
				segmentId,
				type: execResult?.segment.type || "entity",

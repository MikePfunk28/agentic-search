...
export class SegmentExecutor {
	...
	async execute(
		...
		try {
			...
			try {
				...
				console.warn(
					`⚠ [Executor] Model verification failed: ${verifyError instanceof Error ? verifyError.message : verifyError}`,
				);
			...
			console.log(
				`[Executor] Segment ${segment.id} completed in ${executionTime}ms`,
			);
			console.log(
				`[Executor] Found ${searchResults.length} results, confidence: ${findings.confidence.toFixed(2)}`,
			);
			...
			return {
				...
				shouldEscalate,
				newSegments: shouldEscalate
					? this.generateSubSegments(segment, findings)
					: undefined,
			};
		} catch (error) {
	 ...
	 */
	private async verifyAndSelectModel(
		_segment: QuerySegment,
	): Promise<ModelConfig> {
		...
		if (cached) {
			...
			if (age < this.CACHE_TTL_MS) {
				console.log(
					`[Executor] Using cached verification for ${cacheKey} (age: ${Math.floor(age / 1000)}s)`,
				);
				return this.modelConfig;
			} else {
				console.log(
					`[Executor] Cache expired for ${cacheKey}, re-verifying...`,
				);
		...
		try {
			...
			if (response && response.length > 0) {
				...
				console.log(
					`✓ [Executor] Test response: "${response.substring(0, 50)}..."`,
				);
			...
			console.error(
				`✗ [Executor] Model ${cacheKey} verification failed:`,
				error,
			);
			throw new Error(
				`Cannot connect to model ${cacheKey}: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	private buildEnhancedQuery(
		...
		if (Object.keys(dependencyContext).length > 0) {
			...
			for (const context of Object.values(dependencyContext)) {
				...
				const highConfidenceFacts = context.findings.facts.filter(
					(_fact, i) => i < 3,
				); // Top 3 facts per dependency
			...
			if (contextFacts.length > 0) {
				...
				console.log(
					`[Executor] Enhanced query with ${contextFacts.length} context facts`,
				);
			}
		}
	...
	private extractFindings(
		...
		for (const result of results) {
			...
			const entityMatches = result.snippet?.match(
				/[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*|React|Vue|Node|Python/g,
			);
			if (entityMatches) {
		...
		const avgScore =
			results.reduce((sum, r) => sum + (r.addScore || 0), 0) /
			(results.length || 1);
	...
	private shouldEscalate(
		findings: SegmentContext["findings"],
		segment: QuerySegment,
	): boolean {
		// Escalate if:
		...
		if (findings.confidence < 0.5) {
			console.log(
				`[Executor] Low confidence (${findings.confidence.toFixed(2)}) - escalation recommended`,
			);
			return true;
		}
		...
		if (findings.contradictions.length > 0) {
			console.log(
				`[Executor] Contradictions detected - escalation recommended`,
			);
			return true;
		}

		if (
			segment.type === "synthesis" &&
			segment.estimatedComplexity !== "large"
		) {
			console.log(
				`[Executor] Synthesis segment with non-large model - escalation recommended`,
			);
			return true;
		}

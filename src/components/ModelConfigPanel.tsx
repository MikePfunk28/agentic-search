...
export function ModelConfigPanel() {
	...
	const [connectionStatus, setConnectionStatus] = useState<
		Record<string, "connected" | "disconnected" | "unknown">
	>({});
	...
	const fetchModelsForProvider = async (
		...
		try {
			...
			console.log(
				`[ModelConfig] Fetched ${models.length} models for ${provider}`,
			);
		} catch (error) {
		...
		<div className="space-y-6">
						...
						{status === "connected" && models.length > 0 && !isEditing && (
							<div className="mt-3 pt-3 border-t border-slate-700">
								<div className="flex items-center justify-between mb-2">
									...
									>
										<RefreshCw
											className={`w-3 h-3 ${fetchingModels[config.provider] ? "animate-spin" : ""}`}
										/>
										Refresh
									</button>
								</div>
							...
							<div className="mt-3 pt-3 border-t border-red-500/20">
								<p className="text-sm text-red-400">
									...
									{config.requiresApiKey
										...
										: "Make sure the service is running locally."}
								</p>
							</div>
						)}
					</div>
			...
			<div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
				...
				<ul className="text-xs text-gray-300 space-y-1">
					<li>
						• API keys are stored encrypted in your browser's localStorage
					</li>
					<li>
						• Keys are never sent to our servers - all API calls go directly to
						the providers
					</li>
					<li>
						• For local models (Ollama, LM Studio), no API key is required
					</li>
				</ul>
			</div>
		</div>

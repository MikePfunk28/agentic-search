...
export function ReasoningVisualization({
	...
	const totalExecutionTime = session.steps.reduce(
		(sum, step) => sum + (step.result?.executionTimeMs || 0),
		0,
	);
				...
				<div className="grid grid-cols-4 gap-4 mt-4">
					...
					{session.compressionRatio > 1 && (
						<div className="bg-slate-800/50 rounded p-3">
							...
							<div className="text-white text-xl font-bold">
								{session.compressionRatio.toFixed(1)}x
							</div>
						</div>
					)}
				</div>
								...
								<div className="flex items-start gap-4">
									...
									<div className="flex-1">
										<div className="flex items-center gap-2 mb-2">
											<span
												className={`px-2 py-1 rounded text-xs font-medium border ${getStepTypeColor(step.type)}`}
											>

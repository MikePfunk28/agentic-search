...
export function SegmentApprovalUI({
	...
	const getComplexityBadge = (
		complexity: QuerySegment["estimatedComplexity"],
	) => {
		const badges = {
			...
			<div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
				<div className="flex items-center gap-2 mb-3">
					...
					<h3 className="text-lg font-semibold text-white">
						Review Query Segmentation
					</h3>
				</div>
				<p className="text-gray-400 text-sm">
					Review the AI's breakdown of your query. Approve accurate segments or
					provide corrections to improve future results.
				</p>
			</div>
							...
							<div className="p-4">
								<div className="flex items-start justify-between gap-4">
									<div className="flex-1">
										<div className="flex items-center gap-2 mb-2">
											...
											<span
												className={`px-2 py-1 rounded text-xs font-medium ${complexityBadge.color}`}
											>
								...
								{!isEditing && (
									<div className="mt-3 flex gap-2">
										<button
											onClick={() =>
												onApprove(segment.id, true, feedback[segment.id])
											}
										...
										<button
											onClick={() =>
												onApprove(segment.id, false, feedback[segment.id])
											}

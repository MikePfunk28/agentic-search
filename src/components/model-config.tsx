...
export function ModelSettings({ onSave }: ModelSettingsProps) {
		...
		try {
			...
			if (selectedProvider === "ollama" || selectedProvider === "lm_studio") {
				...
				const response = await fetch("/api/detect-models", {
					...
					body: JSON.stringify({
						provider:
							selectedProvider === "lm_studio" ? "lmstudio" : selectedProvider,
						baseUrl: url,
				...
				if (response.ok && !data.error) {
					setTestResult({
						success: true,
						message: `Connection successful! Found ${data.models?.length ?? 0} model(s).`,
					});
				} else {
					setTestResult({
						success: false,
				...
				if (!apiKey) {
					...
					setTestResult({
						success: true,
						message: "Configuration looks good! (Full test on first use)",
					});
				}
			}
		} catch (error) {
					...
					<div>
						...
						<ul className="text-gray-300 text-sm space-y-2">
							<li className="flex gap-2">
								...
								<span>
									<strong>Local model:</strong> Install Ollama
									(http://localhost:11434) or LM Studio
								</span>
							</li>
							<li className="flex gap-2">
								<span className="text-cyan-400">•</span>
								<span>
									<strong>Cloud API:</strong> Add API keys for OpenAI,
									Anthropic, or Google below
								</span>
							</li>
						</ul>
					</div>
	...
	return (
		<div className="space-y-6">
			...
			<div>
				...
				{availableModels.length === 0 && selectedProvider && (
					<p className="mt-2 text-sm text-amber-400">
						Failed to load models. Check if{" "}
						{getProviderDisplayName(selectedProvider)} is running.
					</p>
				)}
			</div>
			...
			{selectedProvider &&
				!["ollama", "lmstudio", "vllm", "gguf", "onnx"].includes(
					selectedProvider,
				) && (
					<div>
						<label className="block text-sm font-medium text-gray-300 mb-2">
			...
			{testResult && (
				...
				>
					...
					<span
						className={testResult.success ? "text-green-100" : "text-red-100"}
					>
						{testResult.message}
					</span>
				</div>

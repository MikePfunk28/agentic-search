...
import { User, Key, Settings, BarChart3, Shield, LogOut, Trash2 } from "lucide-react";
...
function ProfilePage() {
  const { user, isAuthenticated, isLoading, signOut, isAnonymous } = useAppAuth();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-6">Your saved configurations, API keys, and usage stats will appear here.</p>
        ...
        {isAnonymous && (
          <div className="bg-amber-900/30 border border-amber-700/50 rounded-xl p-4">
            <p className="text-amber-300 font-medium">
              You&apos;re signed in as a guest. Your data will be deleted when your session ends.
            </p>
  ...
  return (
    <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
      ...
      <div className="flex items-center gap-4">
        {user?.image ? (
          <img src={user.image} alt={displayName} className="w-16 h-16 rounded-full border-2 border-cyan-500" />
        ) : (
        ...
        <div>
          ...
          {user?.isAnonymous && <span className="text-xs px-2 py-0.5 bg-amber-900/50 text-amber-300 rounded-full">Guest</span>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
      ...
      {apiKeys === undefined ? (
        ...
        <div className="text-gray-500 text-sm">
          ...
          <Link to="/settings" className="text-cyan-400 hover:underline">Settings</Link>.
        </div>
      ) : (
        <div className="space-y-3">
          {apiKeys.map((key) => (
            <div key={key.configId} className="flex items-center justify-between bg-gray-900/50 rounded-lg p-3">
              <div>
                <code className="text-sm text-gray-300 font-mono">{key.masked}</code>
...
function PreferencesCard() {
  ...
  const handleToggle = async (field: "searchHistory" | "analytics", value: boolean) => {
    setSaving(true);
    try {
      ...
      <div className="space-y-4">
        <label className="flex items-center justify-between cursor-pointer">
          <div>
            ...
            <p className="text-sm text-gray-400">Save your search history for later reference</p>
          </div>
          <input
            type="checkbox"
            checked={preferences?.searchHistory ?? true}
            onChange={(e) => void handleToggle("searchHistory", e.target.checked)}
            disabled={saving}
        ...
        <label className="flex items-center justify-between cursor-pointer">
          <div>
            ...
            <p className="text-sm text-gray-400">Track token usage and costs for your dashboard</p>
          </div>
          <input
            type="checkbox"
  ...
  return (
    <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
      <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
        ...
        <Link to="/analytics" className="text-sm text-cyan-400 hover:underline ml-auto font-normal">
          View Full Analytics →
        </Link>
      </h3>
      ...
      {costSummary === undefined ? (
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-gray-900/50 rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-white">{costSummary.totalRequests.toLocaleString()}</p>
            <p className="text-sm text-gray-400">API Calls</p>
          </div>
          <div className="bg-gray-900/50 rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-white">{costSummary.totalTokens.toLocaleString()}</p>
            <p className="text-sm text-gray-400">Tokens Used</p>
          </div>
          <div className="bg-gray-900/50 rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-cyan-400">${costSummary.totalCost.toFixed(4)}</p>
